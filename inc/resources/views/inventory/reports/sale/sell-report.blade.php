...
<div class="content-wrapper">
    <section class="content-header">
              ...
              <div class="card">
                ...
                <div class="card-body">
                  <table id="example" class="display nowrap" style="width:100%">
                      ...
                      <tbody>
                          @foreach ($sell_reports as $item)
                          @php
                            $total = App\Models\ProductInventory::getTotalDebit($item->invoice_no);
                            $total_amount = $total -  App\Models\ProductInventory::get_discount_amount($item->invoice_no);
                          @endphp
                          <tr>
                              <td>{{ $loop->iteration }}</td>
                              <td>{{ $item->invoice_no }}</td>
                              <td>{{ $item->issue_date }}</td>
                              <td>{{ $item->getCustomerInfo->cus_name }}</td>
                              ...
                              <td><div class="btn-group">
                                  ...
                                  <div class="dropdown-menu" role="menu">
                                    <a class="dropdown-item" onclick="sell_reports('{{ $item->invoice_no }}')" href="#">Details</a>
      ...
      <div class="modal fade" id="sellReports" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
let url = "{{ route('inventory.reports.sale.sell-reports-ajax') }}";
